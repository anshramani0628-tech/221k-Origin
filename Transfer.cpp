...
void autonomous() {
    ...
    chassis.moveToPoint(0, 34, 1000, {.maxSpeed=80}); //go to match loader
    chassis.turnToHeading(-90, 1000);; //turn to match loader

    scraper.set_value(true); //scraper down
    

    chassis.moveToPoint(-24,24 ,850); //into match loader
    chassis.waitUntilDone();
    intake.move(127);
    Indexermotor.move(127); 
    pros::delay(550); //intake until 3 balls

    chassis.moveToPoint(5, 24, 1000, {.forwards = false, .maxSpeed=60}); //move back into long goal (not aligned fully)
    chassis.turnToHeading(-90, 500);
    
    chassis.waitUntilDone();
    scraper.set_value(false);

    Indexermotor.move(-127); //get top ball away from nope
    pros::delay(50);
    Indexermotor.move(0);
